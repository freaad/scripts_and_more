...
if [ -z "${CAFFE_ID}" ]; then
    ...
    nvidia-docker run -it --privileged --network=host -p 8888:8888 -p 5000:5000 -v ${CAFFE_HOST_DATA_DIR}:/data/:rw -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=unix${DISPLAY} --name=${CAFFE_NAME} -e PASSWORD="p" bvlc/caffe:gpu bash
else
