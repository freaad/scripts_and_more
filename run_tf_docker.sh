...
if [ -z "${TF_ID}" ]; then
    ...
    nvidia-docker run -it --privileged -p 8888:8888 -p 6006:6006 -v ${TF_HOST_DATA_DIR}:/data/:rw -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=unix${DISPLAY} --name=${TF_NAME} -e PASSWORD="p" gcr.io/tensorflow/tensorflow:latest-gpu bash
else
